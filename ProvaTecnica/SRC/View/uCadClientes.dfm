object frmCad_Clientes: TfrmCad_Clientes
  Left = 0
  Top = 0
  ActiveControl = DBEdtCodigo
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Cadastro de Clientes'
  ClientHeight = 676
  ClientWidth = 910
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object pn_Clientes: TPanel
    Left = 0
    Top = 0
    Width = 910
    Height = 676
    Align = alClient
    BevelOuter = bvNone
    Color = clWhite
    ParentBackground = False
    TabOrder = 0
    ExplicitHeight = 626
    object Panel1: TPanel
      Left = 0
      Top = 0
      Width = 910
      Height = 57
      Align = alTop
      Alignment = taLeftJustify
      BevelOuter = bvNone
      Caption = '  Cadastro de Clientes'
      Color = 16735067
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -21
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 0
    end
    object pn_Grid: TPanel
      Left = 0
      Top = 474
      Width = 910
      Height = 202
      Align = alBottom
      BevelOuter = bvNone
      TabOrder = 1
      ExplicitTop = 424
      object DBG_Clientes: TDBGrid
        Left = 0
        Top = 0
        Width = 910
        Height = 202
        Align = alClient
        BorderStyle = bsNone
        DataSource = ds_CadClientes
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ParentFont = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -13
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = [fsBold]
        OnCellClick = DBG_ClientesCellClick
        OnDrawDataCell = DBG_ClientesDrawDataCell
        OnDrawColumnCell = DBG_ClientesDrawColumnCell
        OnKeyUp = DBG_ClientesKeyUp
        Columns = <
          item
            Expanded = False
            FieldName = 'CODIGO'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOME'
            Width = 730
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'TIPO'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CPFCNPJ'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'RG'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DATACADASTRO'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FLG_ATIVO'
            Visible = True
          end>
      end
    end
    object pn_Controls: TPanel
      Left = 0
      Top = 57
      Width = 910
      Height = 417
      Align = alClient
      BevelOuter = bvNone
      TabOrder = 2
      ExplicitHeight = 367
      object Shape1: TShape
        Left = 10
        Top = 4
        Width = 885
        Height = 336
        Brush.Color = 16770736
        Pen.Style = psClear
        Shape = stRoundRect
      end
      object Label1: TLabel
        Left = 20
        Top = 18
        Width = 43
        Height = 16
        Caption = 'C'#243'digo'
        FocusControl = DBEdtCodigo
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label2: TLabel
        Left = 91
        Top = 18
        Width = 103
        Height = 16
        Caption = 'Nome do Cliente'
        FocusControl = DBEdtNomeCli
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label4: TLabel
        Left = 448
        Top = 19
        Width = 58
        Height = 16
        Caption = 'CPF/Cnpj'
        FocusControl = DBEdtEndereco
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Shape2: TShape
        Left = 11
        Top = 346
        Width = 885
        Height = 65
        Brush.Color = 16770736
        Pen.Style = psClear
        Shape = stRoundRect
      end
      object Label9: TLabel
        Left = 25
        Top = 354
        Width = 72
        Height = 16
        Caption = 'Pesquisar :'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label3: TLabel
        Left = 633
        Top = 23
        Width = 16
        Height = 13
        Caption = 'RG'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label5: TLabel
        Left = 784
        Top = 23
        Width = 81
        Height = 13
        Caption = 'Data Cadastro'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label6: TLabel
        Left = 324
        Top = 66
        Width = 49
        Height = 13
        Caption = 'Telefone'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object DBEdtCodigo: TDBEdit
        Left = 22
        Top = 38
        Width = 59
        Height = 21
        DataField = 'CODIGO'
        DataSource = ds_CadClientes
        ReadOnly = True
        TabOrder = 0
      end
      object DBEdtNomeCli: TDBEdit
        Left = 90
        Top = 38
        Width = 352
        Height = 21
        DataField = 'NOME'
        DataSource = ds_CadClientes
        TabOrder = 1
      end
      object DBEdtEndereco: TDBEdit
        Left = 448
        Top = 38
        Width = 178
        Height = 21
        DataField = 'CPFCNPJ'
        DataSource = ds_CadClientes
        TabOrder = 2
      end
      object EdtPesquisa: TEdit
        Left = 24
        Top = 374
        Width = 425
        Height = 21
        TabOrder = 9
      end
      object DBButtons: TDBNavigator
        Left = 615
        Top = 361
        Width = 261
        Height = 40
        DataSource = ds_CadClientes
        VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast, nbInsert, nbDelete, nbEdit, nbPost, nbCancel]
        Flat = True
        Ctl3D = True
        Hints.Strings = (
          'Primeiro registro'
          'Registro anterior'
          'Pr'#243'ximo registro'
          'Ultimo registro'
          'Inserir registro'
          'Deletar registro'
          'Editar registro'
          'Grava registro'
          'Cencela edi'#231#227'o'
          '')
        ParentCtl3D = False
        ParentShowHint = False
        ConfirmDelete = False
        ShowHint = True
        TabOrder = 10
      end
      object btnConsultar: TButtonGradient
        Left = 455
        Top = 371
        Width = 24
        Height = 24
        ImageIndex = 2
        Images = ImageListIco
        TabOrder = 11
        OnClick = btnConsultarClick
      end
      object DBEditRG: TDBEdit
        Left = 631
        Top = 38
        Width = 148
        Height = 21
        DataField = 'RG'
        DataSource = ds_CadClientes
        TabOrder = 3
      end
      object DTP_DataCadstro: TDateTimePicker
        Left = 784
        Top = 38
        Width = 98
        Height = 21
        Date = 45188.400946215280000000
        Time = 45188.400946215280000000
        TabOrder = 4
      end
      object dbg_TelefoneCLI: TDBGrid
        Left = 325
        Top = 82
        Width = 557
        Height = 65
        DataSource = ds_TelefoneCLI
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 7
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'TELEFONE_CLIENTE'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -12
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 527
            Visible = True
          end>
      end
      object DBRadioGroup1: TDBRadioGroup
        Left = 188
        Top = 82
        Width = 128
        Height = 65
        Color = clWhite
        Columns = 2
        DataField = 'FLG_ATIVO'
        DataSource = ds_CadClientes
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Items.Strings = (
          'Ativo'
          'Inativo')
        ParentBackground = False
        ParentColor = False
        ParentFont = False
        TabOrder = 6
        Values.Strings = (
          '0'
          '1')
      end
      object DBG_TipoPessoa: TDBRadioGroup
        Left = 20
        Top = 82
        Width = 161
        Height = 65
        Caption = 'Tipo de Pessoa'
        Color = clWhite
        Columns = 2
        DataField = 'TIPO'
        DataSource = ds_CadClientes
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Items.Strings = (
          'F'#237'sica'
          'Jur'#237'dica')
        ParentBackground = False
        ParentColor = False
        ParentFont = False
        TabOrder = 5
        Values.Strings = (
          'F'
          'J')
      end
      object GroupBox1: TGroupBox
        Left = 23
        Top = 170
        Width = 861
        Height = 151
        Caption = 'Endere'#231'o'
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentBackground = False
        ParentColor = False
        ParentFont = False
        TabOrder = 8
        object btn_BuscaCEP: TSpeedButton
          Left = 111
          Top = 21
          Width = 23
          Height = 22
          Caption = '...'
          OnClick = btn_BuscaCEPClick
        end
        object MSK_Cep: TMaskEdit
          Left = 10
          Top = 21
          Width = 95
          Height = 21
          EditMask = '##.###-###;0; '
          MaxLength = 10
          TabOrder = 0
          Text = ''
        end
        object DBG_Logradouro: TDBGrid
          Left = 140
          Top = 12
          Width = 709
          Height = 105
          DataSource = ds_EnderecoCLI
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 1
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'Tahoma'
          TitleFont.Style = []
          Columns = <
            item
              Expanded = False
              FieldName = 'ENDERECO'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Tahoma'
              Title.Font.Style = [fsBold]
              Width = 350
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NUMERO'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Tahoma'
              Title.Font.Style = [fsBold]
              Width = 60
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'CEP'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Tahoma'
              Title.Font.Style = [fsBold]
              Width = 78
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'BAIRRO'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Tahoma'
              Title.Font.Style = [fsBold]
              Width = 223
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'CIDADE'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Tahoma'
              Title.Font.Style = [fsBold]
              Width = 199
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'ESTADO'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Tahoma'
              Title.Font.Style = [fsBold]
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'PAIS'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Tahoma'
              Title.Font.Style = [fsBold]
              Visible = True
            end>
        end
        object DBNavigator2: TDBNavigator
          Left = 588
          Top = 123
          Width = 261
          Height = 18
          DataSource = ds_EnderecoCLI
          VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast, nbInsert, nbDelete, nbEdit, nbPost, nbCancel]
          Flat = True
          Ctl3D = True
          Hints.Strings = (
            'Primeiro registro'
            'Registro anterior'
            'Pr'#243'ximo registro'
            'Ultimo registro'
            'Inserir registro'
            'Deletar registro'
            'Editar registro'
            'Grava registro'
            'Cencela edi'#231#227'o'
            '')
          ParentCtl3D = False
          ParentShowHint = False
          ConfirmDelete = False
          ShowHint = True
          TabOrder = 2
        end
      end
      object DBNavigator1: TDBNavigator
        Left = 619
        Top = 147
        Width = 261
        Height = 18
        DataSource = ds_TelefoneCLI
        VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast, nbInsert, nbDelete, nbEdit, nbPost, nbCancel]
        Flat = True
        Ctl3D = True
        Hints.Strings = (
          'Primeiro registro'
          'Registro anterior'
          'Pr'#243'ximo registro'
          'Ultimo registro'
          'Inserir registro'
          'Deletar registro'
          'Editar registro'
          'Grava registro'
          'Cencela edi'#231#227'o'
          '')
        ParentCtl3D = False
        ParentShowHint = False
        ConfirmDelete = False
        ShowHint = True
        TabOrder = 12
      end
      object CKB_SoAtivos: TCheckBox
        Left = 485
        Top = 375
        Width = 77
        Height = 17
        Caption = 'S'#243' Ativos'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 13
        OnClick = CKB_SoAtivosClick
      end
    end
  end
  object ds_CadClientes: TDataSource
    DataSet = TB_Clientes
    OnStateChange = ds_CadClientesStateChange
    Left = 512
    Top = 496
  end
  object TB_Clientes: TClientDataSet
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'CODIGO'
        DataType = ftInteger
      end
      item
        Name = 'NOME'
        DataType = ftString
        Size = 100
      end
      item
        Name = 'TIPO'
        DataType = ftString
        Size = 1
      end
      item
        Name = 'CPFCNPJ'
        DataType = ftString
        Size = 30
      end
      item
        Name = 'RG'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'DATACADASTRO'
        DataType = ftDate
      end
      item
        Name = 'FLG_ATIVO'
        DataType = ftInteger
      end>
    IndexDefs = <>
    Params = <>
    ProviderName = 'DSP_Clientes'
    StoreDefs = True
    BeforeInsert = TB_ClientesBeforeInsert
    AfterInsert = TB_ClientesAfterInsert
    BeforeEdit = TB_ClientesBeforeEdit
    AfterEdit = TB_ClientesAfterEdit
    BeforePost = TB_ClientesBeforePost
    BeforeDelete = TB_ClientesBeforeDelete
    Left = 544
    Top = 496
    object TB_ClientesCODIGO: TIntegerField
      DisplayLabel = 'C'#243'digo'
      DisplayWidth = 13
      FieldName = 'CODIGO'
    end
    object TB_ClientesNOME: TStringField
      DisplayLabel = 'Nome do Cliente'
      DisplayWidth = 100
      FieldName = 'NOME'
      Size = 100
    end
    object TB_ClientesTIPO: TStringField
      DisplayLabel = 'Tipo Pessoa'
      FieldName = 'TIPO'
      Size = 1
    end
    object TB_ClientesCPFCNPJ: TStringField
      DisplayLabel = 'CPF/Cnpj'
      FieldName = 'CPFCNPJ'
      Size = 30
    end
    object TB_ClientesRG: TStringField
      DisplayLabel = 'R.G.'
      FieldName = 'RG'
    end
    object TB_ClientesDATACADASTRO: TDateField
      DisplayLabel = 'Data Cadastro'
      FieldName = 'DATACADASTRO'
      DisplayFormat = '99/99/9999'
      EditMask = '99/99/9999'
    end
    object TB_ClientesFLG_ATIVO: TIntegerField
      DisplayLabel = 'Ativo/Inativo'
      FieldName = 'FLG_ATIVO'
    end
  end
  object ImageListIco: TImageList
    Left = 776
    Top = 321
    Bitmap = {
      
      000000000000}
  end
  object DSP_Clientes: TDataSetProvider
    Left = 584
    Top = 496
  end
  object ds_TelefoneCLI: TDataSource
    DataSet = TB_TelefoneCLI
    Left = 512
    Top = 552
  end
  object TB_TelefoneCLI: TClientDataSet
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'ID'
        DataType = ftInteger
      end
      item
        Name = 'ID_CODCLIENTE'
        DataType = ftInteger
      end
      item
        Name = 'TELEFONE_CLIENTE'
        DataType = ftString
        Size = 20
      end>
    IndexDefs = <>
    Params = <>
    ProviderName = 'DSP_TelefoneCLI'
    StoreDefs = True
    AfterInsert = TB_TelefoneCLIAfterInsert
    AfterEdit = TB_TelefoneCLIAfterEdit
    BeforePost = TB_TelefoneCLIBeforePost
    BeforeDelete = TB_TelefoneCLIBeforeDelete
    Left = 560
    Top = 552
    object TB_TelefoneCLIID: TIntegerField
      FieldName = 'ID'
    end
    object TB_TelefoneCLIID_CODCLIENTE: TIntegerField
      FieldName = 'ID_CODCLIENTE'
    end
    object TB_TelefoneCLITELEFONE_CLIENTE: TStringField
      DisplayLabel = 'Telefone'
      FieldName = 'TELEFONE_CLIENTE'
      EditMask = '!\(99\)00000-0000;0; '
    end
  end
  object ds_EnderecoCLI: TDataSource
    DataSet = TB_EnderecoCLI
    Left = 504
    Top = 608
  end
  object TB_EnderecoCLI: TClientDataSet
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'ID'
        DataType = ftInteger
      end
      item
        Name = 'ID_CODCLIENTE'
        DataType = ftInteger
      end
      item
        Name = 'ENDERECO'
        DataType = ftString
        Size = 100
      end
      item
        Name = 'NUMERO'
        DataType = ftString
        Size = 10
      end
      item
        Name = 'CEP'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'BAIRRO'
        DataType = ftString
        Size = 60
      end
      item
        Name = 'CIDADE'
        DataType = ftString
        Size = 60
      end
      item
        Name = 'ESTADO'
        DataType = ftString
        Size = 10
      end
      item
        Name = 'PAIS'
        DataType = ftString
        Size = 10
      end>
    IndexDefs = <>
    Params = <>
    ProviderName = 'DSP_EnderecoCLI'
    StoreDefs = True
    AfterInsert = TB_EnderecoCLIAfterInsert
    AfterEdit = TB_EnderecoCLIAfterEdit
    BeforePost = TB_EnderecoCLIBeforePost
    BeforeDelete = TB_EnderecoCLIBeforeDelete
    Left = 552
    Top = 608
    object TB_EnderecoCLIID: TIntegerField
      FieldName = 'ID'
    end
    object TB_EnderecoCLIID_CODCLIENTE: TIntegerField
      FieldName = 'ID_CODCLIENTE'
    end
    object TB_EnderecoCLIENDERECO: TStringField
      DisplayLabel = 'Endere'#231'o'
      FieldName = 'ENDERECO'
      Size = 100
    end
    object TB_EnderecoCLINUMERO: TStringField
      DisplayLabel = 'N'#250'mero'
      FieldName = 'NUMERO'
      Size = 10
    end
    object TB_EnderecoCLICEP: TStringField
      FieldName = 'CEP'
      EditMask = '##.###-###;0; '
    end
    object TB_EnderecoCLIBAIRRO: TStringField
      DisplayLabel = 'Bairro'
      FieldName = 'BAIRRO'
      Size = 60
    end
    object TB_EnderecoCLICIDADE: TStringField
      DisplayLabel = 'Cidade'
      FieldName = 'CIDADE'
      Size = 60
    end
    object TB_EnderecoCLIESTADO: TStringField
      DisplayLabel = 'Estado'
      FieldName = 'ESTADO'
      Size = 10
    end
    object TB_EnderecoCLIPAIS: TStringField
      DisplayLabel = 'Pais'
      FieldName = 'PAIS'
      Size = 10
    end
  end
  object RESTClient1: TRESTClient
    Params = <>
    HandleRedirects = True
    Left = 40
    Top = 458
  end
  object RESTRequest1: TRESTRequest
    Client = RESTClient1
    Params = <>
    Response = RESTResponse1
    SynchronizedEvents = False
    Left = 74
    Top = 458
  end
  object RESTResponse1: TRESTResponse
    Left = 106
    Top = 458
  end
  object RESTResponseDataSetAdapter: TRESTResponseDataSetAdapter
    FieldDefs = <>
    Response = RESTResponse1
    Left = 138
    Top = 458
  end
  object Tb_Consulta: TClientDataSet
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'CODIGO'
        DataType = ftInteger
      end
      item
        Name = 'NOME'
        DataType = ftString
        Size = 100
      end
      item
        Name = 'TIPO'
        DataType = ftString
        Size = 1
      end
      item
        Name = 'CPFCNPJ'
        DataType = ftString
        Size = 30
      end
      item
        Name = 'RG'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'DATACADASTRO'
        DataType = ftDate
      end
      item
        Name = 'FLG_ATIVO'
        DataType = ftInteger
      end>
    IndexDefs = <>
    Params = <>
    ProviderName = 'DSP_Clientes'
    StoreDefs = True
    Left = 768
    Top = 496
  end
  object DSP_Consulta: TDataSetProvider
    Left = 808
    Top = 496
  end
  object DSP_TelefoneCLI: TDataSetProvider
    Left = 616
    Top = 552
  end
  object DSP_EnderecoCLI: TDataSetProvider
    Left = 608
    Top = 608
  end
end
