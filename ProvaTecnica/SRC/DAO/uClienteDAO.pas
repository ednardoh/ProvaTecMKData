...
begin
  ...
  VQry.Open('select codigo, nome, Tipo, CPFCNPJ, RG, DataCadastro, FLG_ATIVO from CLIENTES where codigo = '+AValorpesquisa);

  Result := VQry;
end;

function TClienteDao.ObterPorCPFCNPJ(AValor: string): TFDQuery;
var
  VQry: TFDQuery;
begin
  VQry := FConexao.CriarQuery();

  VQry.Open('select codigo, nome, Tipo, CPFCNPJ, RG, DataCadastro, FLG_ATIVO from CLIENTES where CPFCNPJ = '+AValor);

  Result := VQry;
end;

function TClienteDao.ObterSoAtivos: TFDQuery;
var
  VQry: TFDQuery;
begin
  VQry := FConexao.CriarQuery();

  VQry.Open('select codigo, nome, Tipo, CPFCNPJ, RG, DataCadastro, FLG_ATIVO from CLIENTES where FLG_ATIVO = 0');
